  ...
  return (
    <>
      <AppView
        flex
        stretch
        backgroundColor={isFilter ? '#2302022E' : '#FAF9FA'}>
        {isSearch ? renderSearch() : header()}
        <AppView height={2} />
        {sections.length > 0 ? (
          <AppTabs showUnderLine scrollableTabBar={true} sections={sections}>
            {sections.map((item, index) => (
              <StoreList
                ...
                subCatID={item.id}
                word={word}
                label={lang.lang == 'ar' ? item.ar.name : item.en.name}
                nearBy={nearBy}
                isFilter={isFilter}
                setNumber={setNumber}
              />
