...
class Form extends PureComponent {
  static propTypes = {
    ...
    extraValidationSchema: PropTypes.func,
  };
  ...
  validateFormField = name => {
    ...
    if (
      name === 'new_password' &&
      this.values['new_password'] === this.values['new_password_confirmation']
    ) {
      const errorsCollection = {...this.state.errors};
      delete errorsCollection['new_password_confirmation'];
      this.setState({
        errors: errorsCollection,
      });
    } else if (
      name === 'new_password' &&
      this.values['new_password_confirmation'] !== '' &&
      this.values['new_password'] !== this.values['new_password_confirmation']
    ) {
      const error1 = this.validateField('new_password_confirmation');
      this.setState({
        errors: {
          ...this.state.errors,
          new_password_confirmation: error1['new_password_confirmation'],
        },
      });
    }
    if (error[name]) {
